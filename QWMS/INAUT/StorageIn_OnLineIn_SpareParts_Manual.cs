...
namespace QWMS
{
    public partial class StorageIn_OnLineIn_SpareParts_Manual : Form
    {
        #region 變數宣告
        ...
        public StorageIn_OnLineIn_SpareParts_Manual()
        {
            InitializeComponent();
        }
        public StorageIn_OnLineIn_SpareParts_Manual(ref UserInfo varUserData, string strProgid)
        {
        ...
        private void btnSave_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dtData.Rows.Count; i++)
                {
                    ...
                    #region //宣告Struct陣列
                    sct.Location = new string[dtData.Rows.Count];//儲位
                    sct.Part = new string[dtData.Rows.Count];//料號
                    sct.PoNo = new string[dtData.Rows.Count];//PO No.
                    sct.Kdmat = new string[dtData.Rows.Count];//客人料號
                    sct.Version = new string[dtData.Rows.Count];//版本
                    #endregion
                    ...
                    for (int j = 0; j < dtData.Rows.Count; j++)
                    {
                        ...
                        {
                            stsWarning.Text = "相同的料號:" + dtData.Rows[j]["MATNR"].ToString() + " 存在不同的PO No.，請放在不同的儲位!!";
                            SetbtnSaveException();
                            return;
                        }
                    }
                    ...
                    for (int j = 0; j < dtData.Rows.Count; j++)
                    {
                        ...
                        {
                            stsWarning.Text = "相同的料號:" + dtData.Rows[j]["MATNR"].ToString() + " 存在不同的客人料號，請放在不同的儲位!!";
                            SetbtnSaveException();
                            return;
                        }
                    }
                    ...
                    for (int j = 0; j < dtData.Rows.Count; j++)
                    {
                        ...
                        {
                            stsWarning.Text = "相同的料號:" + dtData.Rows[j]["MATNR"].ToString() + " 存在不同的版本，請放在不同的儲位!!";
                            SetbtnSaveException();
                            return;
                        }
                    }
                    ...
                    #region  //檢查要入庫的單號之前是否有使用連板入庫的方式入庫
                    QCI.QWMS.StorageData objStorageData = new QCI.QWMS.StorageData(CommonInfo.Instance.DBType, CommonInfo.Instance.DBCode.ToString(), CommonInfo.Instance.ErrType, CommonInfo.Instance.ErrCode.ToString(), UserData, Werks, Lgort);
                    ...
                    #region  //檢查要入庫的儲位是有相同的料號但不同連板資料(包含單板)
                    ...
                    #region 提示PO No.與客人料號(Customer P/N)未輸入的訊息

                    if (dtData.Rows[i]["EBELN"].ToString() == "")
                    {
                        stsWarning.Text = "PO No.為空值!!";
                    }
                    else if (dtData.Rows[i]["KDMAT"].ToString() == "")
                    {
                        stsWarning.Text = "客人料號(Customer P/N)為空值!!";
                    }
        ...
        public bool CheckDifferentPoNo(int j)
        {
            ...
            for (int k = 0; k < TotalLocation; k++)
            {
                ...
                if (sct.Location[k].ToString() == dtData.Rows[j]["LOCAT"].ToString() && sct.Part[k].ToString() == dtData.Rows[j]["MATNR"].ToString())
                {
                    //PO不同，需放不同的儲位：return false
                    if (sct.PoNo[k].ToString() != dtData.Rows[j]["EBELN"].ToString())
                    {
                        chkFlag = false;
                        break;
                    }
        ...
        public bool CheckDifferentCustomerPN(int j)
        {
            ...
            for (int k = 0; k < TotalLocation; k++)
            {
                ...
                if (sct.Location[k].ToString() == dtData.Rows[j]["LOCAT"].ToString() && sct.Part[k].ToString() == dtData.Rows[j]["MATNR"].ToString())
                {
                    ...
                    if (sct.Kdmat[k].ToString() != dtData.Rows[j]["KDMAT"].ToString())
                    {
                        chkFlag = false;
                        break;
                    }
        ...
        public bool CheckDifferentVersion(int j)
        {
            ...
            for (int k = 0; k < TotalLocation; k++)
            {
                ...
                if (sct.Location[k].ToString() == dtData.Rows[j]["LOCAT"].ToString() && sct.Part[k].ToString() == dtData.Rows[j]["MATNR"].ToString())
                {
                    ...
                    if (sct.Version[k].ToString() != dtData.Rows[j]["CHARG"].ToString())
                    {
                        chkFlag = false;
                        break;
                    }
        ...
        {
            sct.Location[p] = dtData.Rows[q]["LOCAT"].ToString();
            sct.Part[p] = dtData.Rows[q]["MATNR"].ToString();
            if (strVar == "Ebeln")
            {
                sct.PoNo[p] = dtData.Rows[q]["EBELN"].ToString();
            }
            else if (strVar == "Kdmat")
            {
                sct.Kdmat[p] = dtData.Rows[q]["KDMAT"].ToString();
            }
            else if (strVar == "Charg")
            {
                sct.Version[p] = dtData.Rows[q]["CHARG"].ToString();
            }
        }
        #endregion
        ...
        public void btnAdd_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (CheckIsOpen("objStorageIn_OnLineIn_SpareParts_Add"))
                {
