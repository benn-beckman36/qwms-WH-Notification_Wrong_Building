...
{
	/// <summary>
	/// Alim_MaterialSelect 的摘要描述。
	/// </summary>
	public class Alim_MaterialSelect : System.Windows.Forms.Form
	{
		...
		private AlimStorageQuery objAlimStorageData;
        ...
        public Alim_MaterialSelect(UserInfo _UserData, string strProgid, string strWerks, string strLgort, string strType, string strTempMatnr)
		{
            UserData = _UserData;
			...
			try
			{
                ...
                objAlimStorageData = new AlimStorageQuery(UserData);
				ShowMatnrData();
			}
			catch(Exception ex)
			{
		...
		private void ShowMatnrData()
		{
			try
			{
                dtData = objAlimStorageData.QueryPartData(Werks, Lgort, strTempMatnr, Type);
				ShowDataGrid();
			}
			catch(Exception ex)
			{
