...
{
    /// <summary>
    /// StorageOut_SAP_Simulation 的摘要描述。
    /// </summary>
    public partial class StorageOut_SAP_Simulation : System.Windows.Forms.Form
    {
        #region 變數宣告
        ...
        public StorageOut_SAP_Simulation(UserInfo varUserData, string strProgid)
        {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dtSmtData.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtTempStorage.Rows.Count; i++)
                    {
                        ...
                        if (alAllCombine.IndexOf(sbCombineIndex.ToString()) < 0)
                        {
                            ...
                            //假如要發的數量大於QWMS的庫存量，則只發出QWMS實際的庫存量
                ...
                else if (dtFinData.Rows.Count > 0)
                {
                    ...
                    drNoneZero = dtFinalStorage.Select("MENGE <> '0'");
                    for (int k = 0; k < drNoneZero.Length; k++)
                    {
                    ...
                    //將資料依料號、線別、工單排序
                    strOderBy = "MATNR, ARBPL, WKORD";
                    dtFinalStorage = CommonInfo.SortDataTable(dtFinalStorage, strOderBy);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbType.Text == "SMT")
                {
                    ...
                    if (objStorageOut.AddSapSimulationData(dtSmtSave, Type, Category))
                    {
                ...
                else if (cmbType.Text == "FINAL")
                {
                    ...
                    if (objStorageOut.AddSapSimulationData(dtFinSave, Type, Category))
                    {
                        stsWarning.Text = "Update OK!!";
                        this.btnQuery.Enabled = false;
                        this.btnSave.Enabled = false;
        ...
        {
            strQwms = cmbQwms.Items[cmbQwms.SelectedIndex].ToString();
        }
