...
namespace QWMS
{
    public partial class StorageOut_OnLineOut_SpareParts : Form
    {
        #region 變數宣告
        ...
        public StorageOut_OnLineOut_SpareParts()
        {
            InitializeComponent();
        }

        public StorageOut_OnLineOut_SpareParts(UserInfo varUserData, string strProgid)
		{
        ...
        private void txtMblnr_DoubleClick(object sender, System.EventArgs e)
        {
            try
            {
                ...
                StorageOut_SapDataSelect objStorageOut_SapDataSelect = new StorageOut_SapDataSelect(UserData, Werks, Lgort, Progid, Mblnrs, "SPARE_PARTS");
        ...
        private void ShowOutSourceDataGrid()
        {
            ...
            try
            {
                ...
                if (dtOutSource.Rows.Count > 0)
                {
                    ...
                    this.chkEbeln.Enabled = false;
                    this.chkKdmat.Enabled = false;
        ...
        private void btnConfirm_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                //不管SI單據的倉別，一律預設帶預設的倉別FGSP
                for (int i = 0; i < dtOutSource.Rows.Count; i++)
                {
                    dtOutSource.Rows[i]["LGORT"] = "FGSP";
                }
        ...
        private void btnQuery_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                bool bolNonEbeln = false;
                bool bolNonKdmat = false;
                if (chkEbeln.Checked == false)
                    bolNonEbeln = true;
                if (chkKdmat.Checked == false)
                    bolNonKdmat = true;

                StorageData objStorageData = new StorageData(UserData, Werks, Lgort);
                dsData = objStorageData.QueryOnLineOutData_SpareParts(dtOutSource, "", "", false, "", false, false, bolNonEbeln, bolNonKdmat);  
                ...
                for (int i = 0; i < dtTempStorage.Rows.Count; i++)
                {
                    ...
                    //需另外考慮SI No.的PO No.(WHDWN.EBELN)與客人料號(WHDWN.KDMAT)必須與庫存資料一致  Add by Smose Liao 20100503
                    if (chkEbeln.Checked == false)
                    {
            ...
            try
            {
                ...
                if (dtStorage.Rows.Count > 0)
                {
                    for (int i = 0; i < dtStorage.Rows.Count; i++)
                    {
                        ...
                        //else if (dtStorage.Rows[i]["NLOCA"].ToString().Substring(0, 1) != "N")
                        //{
                        //    stsWarning.Text = "請選擇正確的待出貨儲位，第一個字元應為N!!";
                        //    SetbtnSaveException();
                        //    return;
                        //}
                    }
                ...
                if (objStorageOut.AddSinoOnLineOutData(dtOutSource, dtStorage, dtSave))  
                {
        ...
        private void btnRefresh_Click(object sender, System.EventArgs e)
        {
            ...
            this.chkEbeln.Enabled = true;
            this.chkKdmat.Enabled = true;
            this.chkEbeln.Checked = false;
            this.chkKdmat.Checked = false;
        ...
        //private void dgvStorage_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        //{
        //    //取得目前使用者所點選的游標位置
        //    CurRowIndex = int.Parse(dgvStorage.CurrentRow.Index.ToString());

        //    try
        //    {
        //        StorageIn_LocationSelect objStorageIn_LocationSelect = new StorageIn_LocationSelect(UserData, "B8", Werks, Lgort, CurLocat, "NEW", aryMatnr);
        //        objStorageIn_LocationSelect.ShowDialog();
        //        strLocat = objStorageIn_LocationSelect.Locat;

        //        //Shipment Location(待出貨儲位)：使用者所點選的儲位   Smose Liao 20100428
        //        if (strLocat.Trim() != "")
        //        {
        //            dtStorage.Rows[CurRowIndex]["NLOCA"] = strLocat;
        //            dtStorage.AcceptChanges();

        //            dtCombineStorage.Rows[CurRowIndex]["NLOCA"] = strLocat;
        ...
        private void cmbLocat_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgvStorage.Rows.Count -1; i++)
            {
                ...
                if (dgvStorage.Rows[i].Cells[0].Value.ToString() == "True" )
                {
                    ...
                    //dtCombineStorage.Rows[i]["NLOCA"] = cmbLocat.Items[cmbLocat.SelectedIndex].ToString();
                }
                else
                {
                    rowSelectFalse++;
