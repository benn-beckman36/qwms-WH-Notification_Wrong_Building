...
namespace QWMS
{
    public partial class TransferInMTConfirm : Form
    {
        #region 变量
        ...
        #endregion
        public TransferInMTConfirm(UserInfo varUserData, string strProgid)
        {
            InitializeComponent();
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            if (strStatus == "JR")
            {
                MessageBox.Show("确认信息无误，准许进码头");
                objTransfer.UpdateStatus(lblGRNOOut.Text, Usrnm,strStatus);
                MessageBox.Show("已进入码头");
            }
            
            else
            {
                MessageBox.Show("目前状态是" + dtStatus.Rows[0]["ST"].ToString() + "，不可经过码头，请确认！");
            }
