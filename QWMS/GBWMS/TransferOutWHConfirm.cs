...
namespace QWMS
{
    public partial class TransferOutWHConfirm : Form
    {
        #region 变量
        ...
        #endregion
        public TransferOutWHConfirm(UserInfo varUserData, string strProgid)
        {
            InitializeComponent();
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            if (strStatus == "WW")
            {
                MessageBox.Show("确认信息无误，准许离开仓库");
                objTransfer.UpdateStatus(lblGRNOOut.Text, Usrnm,strStatus);
                MessageBox.Show("离开仓库成功");
            }
            
            else
            {
                MessageBox.Show("目前状态是" + dtStatus.Rows[0]["ST"].ToString() + "，不可离开仓库，请确认！");
            }
