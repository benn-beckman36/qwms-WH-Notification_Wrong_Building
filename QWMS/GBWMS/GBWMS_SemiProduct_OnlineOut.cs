...
namespace QWMS
{
    public partial class GBWMS_SemiProduct_OnlineOut : Form
    {
        #region Constructor

        public GBWMS_SemiProduct_OnlineOut(UserInfo varUserData, string strProgid)
        {
            InitializeComponent();
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //检验BCM出库加扣时是否是同一料号版本的扣帐编号&加上BCM料号
                string[] strMatnrs = new string[dtOutSource.Rows.Count];
                string[] strChargs = new string[dtOutSource.Rows.Count];
                for (int i = 0; i < dtOutSource.Rows.Count; i++)
                {
                    strMatnrs[i] = dtOutSource.Rows[i]["MATNR"].ToString().Trim();
                    strChargs[i] = dtOutSource.Rows[i]["CHARG"].ToString().Trim();                    
                }
                for (int i = 0; i < dtOutSource.Rows.Count - 1; i++)
                {
                    if ((strMatnrs[i] != strMatnrs[i + 1] || strChargs[i] != strChargs[i + 1]) && (strMatnrs[i].Substring(0, 4).ToUpper() == "2CJH" || strMatnrs[i+1].Substring(0, 4).ToUpper() == "2CJH"))
                    {
                        MessageBox.Show("BCM材料请输入相同料号版本的扣帐编号!!");
                        return;
                    }
                    else if (strMatnrs[i] == strMatnrs[i + 1] && (strMatnrs[i].Substring(0, 4).ToUpper() == "20JH" || strMatnrs[i].Substring(0, 4).ToUpper() == "2LJH"))
                    {
                        MessageBox.Show("GB材料请输入不同料号版本的扣帐编号!!");
                        return;
                    }
                }
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dtOutSource.Rows.Count; i++)
                {
                    ...
                    if (dtOutSource.Rows[i]["MATNR"].ToString().Trim().Substring(0, 4) == "20JH" || dtOutSource.Rows[i]["MATNR"].ToString().Trim().Substring(0, 4) == "2LJH")//若为GB
                    {
                        string strUMLGO = dtOutSource.Rows[i]["UMLGO"].ToString().Trim();

                        if (!arrUMLGO.Contains(strUMLGO))
                        {
                            arrUMLGO.Add(strUMLGO);
                        }
                    }
                }
                ...
                if (arrUMLGO.Count > 1)
                {
                    MessageBox.Show("目的仓不一样，不能一起出库", "提醒");
                    return;
                }
                ...
                dtStorageLocation = dsData.Tables[1].Select("CHKED = true").CopyToDataTable();
                //dtStorageLocation = dsData.Tables[1].Copy();//CHKED=TRUE;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (dtOutSource.Rows[0]["MATNR"].ToString().Trim().Substring(0, 4).ToUpper() == "20JH" || dtOutSource.Rows[0]["MATNR"].ToString().Trim().Substring(0, 4).ToUpper() == "2LJH")
            {
                for (int i = 0; i < dtOutSource.Rows.Count; i++)
                {
            ...
            else//Modify BCM out storage by Jack
            {
                for (int i = 0; i < dtOutSource.Rows.Count; i++)
                {
                    int intAlmng = 0;//SAP单据数量
                    string strMatnr = dtOutSource.Rows[i]["MATNR"].ToString().Trim();
                    string strInsmk = dtOutSource.Rows[i]["INSMK"].ToString().Trim();
                    string strCharg = dtOutSource.Rows[i]["CHARG"].ToString().Trim();
                    string strMblnr = dtOutSource.Rows[i]["MBLNR"].ToString().Trim();
                    intAlmng = int.Parse(dtOutSource.Rows[i]["ALMNG"].ToString().Trim());
                    int intAlqty = 0;//备料信息数量
                    DataRow[] drsStorageLocation = dtStorageLocation.Select(string.Format("MATNR = '{0}' AND INSMK = '{1}' AND CHARG = '{2}' AND MBLNR='{3}'", strMatnr, strInsmk, strCharg, strMblnr));
                    for (int j = 0; j < drsStorageLocation.Length; j++)
                    {
                        intAlqty += int.Parse(drsStorageLocation[j]["ALQTY"].ToString().Trim());
                    }
                    ...
                    if (intAlmng != intAlqty)
                    {
                        strMblnr = dtOutSource.Rows[0]["MBLNR"].ToString().Trim();
                        MessageBox.Show(string.Format("備料信息總數量不等於SAP單據的數量，SAP單據：{0}，請確認!!", strMblnr), "提醒");
                        return;
                    }
                }
                //int intAlmng = 0;
                //string strMatnr = dtOutSource.Rows[0]["MATNR"].ToString().Trim();
                //string strInsmk = dtOutSource.Rows[0]["INSMK"].ToString().Trim();
                //string strCharg = dtOutSource.Rows[0]["CHARG"].ToString().Trim();
                //for (int i = 0; i < dtOutSource.Rows.Count; i++)
                //{
                //    intAlmng = int.Parse(dtOutSource.Rows[i]["ALMNG"].ToString().Trim()) + intAlmng;                   
                //}
                ...
                //if (intAlmng != intAlqty)
                //{
                //    string strMblnr = dtOutSource.Rows[0]["MBLNR"].ToString().Trim();
                //    MessageBox.Show(string.Format("備料信息總數量不等於SAP單據的數量，SAP單據：{0}，請確認!!", strMblnr), "提醒");
                //    return;
                //}
            }
            ...
            //string strPrintLocat = null;
            //string strPrintMatnr = null;
            //string strPrintCharg = null;
            //for (int i = dtPrint.Rows.Count - 1; i >= 0; i--)
            //{
            //    if (dtPrint.Rows[i]["LOCAT"].ToString().Trim() != strPrintLocat || dtPrint.Rows[i]["MATNR"].ToString().Trim() != strPrintMatnr || dtPrint.Rows[i]["CHARG"].ToString().Trim() != strPrintCharg)
            //    {
            //        strPrintLocat = dtPrint.Rows[i]["LOCAT"].ToString().Trim();
            //        strPrintMatnr = dtPrint.Rows[i]["MATNR"].ToString().Trim();
            //        strPrintCharg = dtPrint.Rows[i]["CHARG"].ToString().Trim();
            //    }
            //    else
            //    {
            //        dtPrint.Rows[i]["MENGE"] = int.Parse(dtPrint.Rows[i + 1]["MENGE"].ToString().Trim()) + int.Parse(dtPrint.Rows[i]["MENGE"].ToString().Trim());
            //        dtPrint.Rows[i]["ALQTY"] = int.Parse(dtPrint.Rows[i + 1]["ALQTY"].ToString().Trim()) + int.Parse(dtPrint.Rows[i]["ALQTY"].ToString().Trim());
            //        dtPrint.Rows[i]["BLACE"] = int.Parse(dtPrint.Rows[i + 1]["BLACE"].ToString().Trim()) + int.Parse(dtPrint.Rows[i]["BLACE"].ToString().Trim());
            //        dtPrint.Rows.RemoveAt(i + 1);
            //    }
            //    dtPrint.Rows[i]["RMAK1"] = "";
            //}
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!bolNoScan)
                {
                    if (dtStorageLocation.Rows[0]["MATNR"].ToString().Trim().Substring(0, 4) == "20JH" || dtStorageLocation.Rows[0]["MATNR"].ToString().Trim().Substring(0, 4) == "2LJH")//若为GB
                    {
                        for (int i = 0; i < dtStorageLocation.Rows.Count; i++)
                        {
                            int intAlqty = int.Parse(dtStorageLocation.Rows[i]["ALQTY"].ToString().Trim());
                    ...
                    else//若为BCM
                    {
                        ...
                        for (int i = 0; i < dtStorageLocation.Rows.Count; i++)
                        {
                            intAlqty = int.Parse(dtStorageLocation.Rows[i]["ALQTY"].ToString().Trim()) + intAlqty;
                        }
                        for (int j = 0; j < dtStorageLocation.Rows.Count; j++)
                        {
                            intLocScqty = int.Parse(dtStorageLocation.Rows[j]["SCQTY"].ToString().Trim()) + intLocScqty;
                        }
                        if (intAlqty != intLocScqty)
                        {
                            string strLocat = dtStorageLocation.Rows[0]["LOCAT"].ToString().Trim();
                            MessageBox.Show(string.Format("刷入的BOX ID/SN數量不等於備料信息數量，儲位：{0}，請確認!!", strLocat), "提醒");
                            return;
                        }
                        ...
                        {
                            intOutScqty = int.Parse(dtOutSource.Rows[j]["SCQTY"].ToString().Trim()) + intOutScqty;
                        }
                        if (intAlmng != intOutScqty)
                        {
                            string strMblnr = dtOutSource.Rows[0]["MBLNR"].ToString().Trim();
                            MessageBox.Show(string.Format("刷入的BOX ID總數量不等於SAP單據的數量，SAP單據：{0}，請確認!!", strMblnr), "提醒");
                            return;
                ...
                if (objStorageOut.AddSemiProductOut_Confirm(dtOutSource, dtStorage, bolNoScan))
                {

                    stsWarning.Text = "Update OK!!";

                    MessageBox.Show("出库扣帐OK！", "提醒",MessageBoxButtons .OK,MessageBoxIcon.Information);
                ...
                {
                    MessageBox.Show("出库扣帐失败！" + objStorageOut.ERRMSG, "提醒", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void txtSN_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)13)
            {
                if (txtLocat.Text.Trim() == string.Empty)
                {
                        ...
                        if (txtSn.Text.Length > 0)
                        {
                            ...
                            if (txtSn.Text.Trim() == txtfixSn.Text.Trim())
                            {
                                if (!objSapData.GetBoxIDHoldStatusBySn(txtSn.Text.Trim()))
                                {
                                    ...
                                    if (dtTemp.Rows.Count > 0)
                                    {
                                        if (txtLocat.Text.Trim().ToString() != dtTemp.Rows[0]["LOCAT"].ToString().Trim())
                                        {
                                            Sound.Play(@"Sound\OO.wav");
                                            stsWarning.Text = "刷入信息与储位不符！！";
                                            txtSn.Text = string.Empty;
                                            txtSn.Focus();
                                            return;
                                        }
                                        //int outAlqty = int.Parse(dtOutLocation.Rows[0]["ALQTY"].ToString().Trim());
                                        ScanAlqty += int.Parse(dtTemp.Rows[0]["ALQTY"].ToString().Trim());
                                        if (ScanAlqty > outAlqty)
                                        {
                                            Sound.Play(@"Sound\OO.wav");
                                            stsWarning.Text = "刷入数量不能大于出库数量！！";
                                            txtSn.Text = string.Empty;
                                            ScanAlqty -= int.Parse(dtTemp.Rows[0]["ALQTY"].ToString().Trim());
                                            txtSn.Focus();
                                            return;
                                                ...
                                                if (drsStorageLocation.Length > 0)
                                                {
                                                    ...
                                                    for (int i = 0; i < dtTemp.Rows.Count; i++)
                                                    {
                                                        ...
                                                        if (drsSelect.Length > 0)
                                                        {
                                                            ...
                                                            ScanAlqty -= int.Parse(dtTemp.Rows[i]["ALQTY"].ToString().Trim());
                                                            txtSn.Text = string.Empty;
                                                            txtSn.Focus();
                                                            return;
                                                        }
                                                    ...
                                                    {
                                                        Sound.Play(@"Sound\OO.wav");
                                                        stsWarning.Text = txtLocat.Text.Trim().ToString() + "刷入数量已满足备料信息";
                                                        txtSn.Text = string.Empty;
                                                        ScanAlqty = 0;
                                                        outAlqty = 0;
                ...
                else
                {
                    ...
                    if (!objSapData.GetBoxIDHoldStatusBySn(txtSn.Text.Trim()))
                    {
                        ...
                        if (dtTemp.Rows.Count > 0)
                        {
                            ...
                            //outData = dtStorageLocation.Select("LOCAT='" + txtLocat.Text.Trim().ToString() + "' ").CopyToDataTable();
                            //int outAlqty = int.Parse(dtOutLocation.Rows[0]["ALQTY"].ToString().Trim());
                            if (txtLocat.Text.Trim().ToString() != dtTemp.Rows[0]["LOCAT"].ToString().Trim())
                            {
                                Sound.Play(@"Sound\OO.wav");
        ...
        private void txtBoxid_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == System.Windows.Forms.Keys.Enter)
            {
                ...
                try
                {
                    ...
                    {
                        if (txtLocat.Text.Trim().ToString() != dtTmpData.Rows[0]["LOCAT"].ToString().Trim())
                        {
                            Sound.Play(@"Sound\OO.wav");
                            stsWarning.Text = "刷入信息与储位不符！！";
                            txtBoxid.Text = string.Empty;
                        ...
                        ScanAlqty += int.Parse(dtTmpData.Rows[0]["ALQTY"].ToString().Trim());
                        if (ScanAlqty > outAlqty)
                        {
                            Sound.Play(@"Sound\OO.wav");
                            stsWarning.Text = "刷入数量不能大于出库数量！！";
                                ...
                                if (drsStorageLocation.Length > 0)
                                {
                                    ...
                                    for (int i = 0; i < dtTmpData.Rows.Count; i++)
                                    {
                                        ...
                                        if (drsSelected.Length > 0)
                                        {
                                            ...
                                            ScanAlqty -= int.Parse(dtTmpData.Rows[i]["ALQTY"].ToString().Trim());
                                            txtBoxid.Text = string.Empty;
                                            txtBoxid.Focus();
                                            return;
                                        }
        ...
        private void txtLocat_keyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == System.Windows.Forms.Keys.Enter)
            {
                ...
                for (int i = 0; i < dtStorageLocation.Rows.Count; i++)
                {
                    arrLocation.Add(dtStorageLocation.Rows[i]["LOCAT"].ToString().Trim());
                }
                foreach (string strLocat in arrLocation)
                {
                    if(txtLocat.Text.Trim().ToString() == strLocat)
                    {
                        inLocation += 1;
                    }
                }
                if(inLocation > 0)
                {
                    dtOutLocation = dtStorageLocation.Select("LOCAT='"+txtLocat.Text.Trim().ToString()+"'").CopyToDataTable();
                    for (int i = 0; i < dtOutLocation.Rows.Count; i++)
                    {
                        outAlqty += int.Parse(dtOutLocation.Rows[i]["ALQTY"].ToString().Trim());
                    }
        ...
        //                                            drsOutSource[0]["SCQTY"] = Convert.ToInt32(drsOutSource[0]["SCQTY"]) + Convert.ToInt32(dtTemp.Rows[i]["ALQTY"]);//dtStorage.Select(strSelectConditions).Length;
        //                                            drsStorageLocation[0]["SCQTY"] = Convert.ToInt32(drsStorageLocation[0]["SCQTY"]) + Convert.ToInt32(dtTemp.Rows[i]["ALQTY"]);//dtStorage.Select(strSelectConditions).Length;
        //                                            dtTemp.Rows[i]["DOCNO"] = drsStorageLocation[0]["MBLNR"].ToString().Trim();
        ...
        //                            drsOutSource[0]["SCQTY"] = Convert.ToInt32(drsOutSource[0]["SCQTY"]) + Convert.ToInt32(dtTmpData.Rows[i]["ALQTY"]);//dtStorage.Select(strSelectConditions).Length;
        //                            drsStorageLocation[0]["SCQTY"] = Convert.ToInt32(drsStorageLocation[0]["SCQTY"]) + Convert.ToInt32(dtTmpData.Rows[i]["ALQTY"]);//dtStorage.Select(strSelectConditions).Length;
        //                            dtTmpData.Rows[i]["DOCNO"] = drsStorageLocation[0]["MBLNR"].ToString().Trim();
